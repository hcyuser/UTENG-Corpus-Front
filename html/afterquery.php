<?php
    ...
    $rs=read_multi_record($__db, "SELECT distinct id FROM user.professor WHERE quality >= '1' AND quality <= '2' AND school like 'University of Delaware' AND location like 'Newark, DE' into TC; select R.* from user.response R inner join TC on R.tid=TC.id where C_DATE >= '2014-01-01' AND C_DATE <= '2017-12-31'; ",array(),array());
    if ($rs === false){
